...
"""
このファイルはあるしきい値に対してのみシミュレーションを回したい時に作成
"""
        ...
        if (i):
            for e in range(len(eventtrigger)):
                ...
                for t in range(iteration):
                    ...
                    if t==0:
                        objective_function_array.append(initial_objective_function)
                    ...
                    for d in range(len(multi_gp[0].kernel.param)):
                        grad_array[d].append(tmp_grad[d])

                    tmp_normalize_error = [0]*N
                    sum_normalize_error = 0
                    objective_function_array.append(tmp_objective_function)
                    ...
                    normalize_error.append(sum_normalize_error)

                    if (t%1000)==0:            
                        print(str(t) + '回目')
                        print('==========================')
                        print('objective function')
                        print(objective_function_array[-1])
                        # print('==========================')
                        ...
                        print('==========================')
                        for d in range(3):
                            print(grad_array[d][-1])

                # terminal_count[e] = count
                
                if e==0:
                    main(normalize_error[-1], iteration, param0)
                ...
                for d in range(3):
                    print(grad_array[d][-1])
                print(normalize_error[-1])
                
                for d in range(len(multi_gp[0].kernel.param)):
                    np.savetxt(save_dir+'grad_array_'+str(eventtrigger[e])+str(d)+'.dat', grad_array[d])


        if (i):

            color = ['g', 'b', 'r']
            label = ["E="+str(eventtrigger[0])]
            
            plt.figure(figsize=(7,7))
            plt.title('normalize_error')

            for e in range(len(eventtrigger)):
                plt.plot(np.arange(0,iteration+1), np.array(normalize_error)/initial_error, color=color[e], label=label[e])
            plt.yscale('log')
            plt.legend(loc='best')
            ...
            for e in range(len(eventtrigger)):
                plt.plot(np.arange(0,iteration+1), objective_function_array/initial_objective_function, color=color[e], label=label[e])
            # plt.yscale('log')
            plt.legend(loc='best')
            ...
            for e in range(len(eventtrigger)):
                ...
                for j in range(len(multi_gp[0].kernel.param)):
                    plt.plot(np.arange(0,iteration), grad_array[j], color=color[j], label=theta_array[j])
                plt.legend(loc='best')
